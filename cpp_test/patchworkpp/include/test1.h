...
namespace patchwork {
    ...
    struct PointXYZI {
        ...
        float z;
        float intensity;
        PointXYZI(float _x, float _y, float _z, float _inten) : x(_x), y(_y), z(_z), intensity(_inten){}
    };
...
//            std::cout << "PatchWorkpp::PatchWorkpp() - INITIALIZATION COMPLETE" << std::endl;
        ...
        Eigen::MatrixX4f getCenters() { return toEigenCloud(centers_); }
        Eigen::MatrixX4f getNormals() { return toEigenCloud(normals_); }
    ...
    private:
        ...
        vector<PointXYZI> centers_, normals_;
