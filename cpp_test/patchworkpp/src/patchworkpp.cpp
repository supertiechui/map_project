...
void PatchWorkpp::estimate_plane(const vector<PointXYZI> &ground) {
    ...
    Eigen::MatrixX3f centered = eigen_ground.block(0,0,eigen_ground.rows(),3).rowwise() - eigen_ground.block(0,0,eigen_ground.rows(),3).colwise().mean();
    ...
    for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
                ...
                centers_.push_back(PointXYZI(pc_mean_(0), pc_mean_(1), pc_mean_(2), 0));
                normals_.push_back(PointXYZI(normal_(0), normal_(1), normal_(2), 0));
