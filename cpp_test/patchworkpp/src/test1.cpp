...
void PatchWorkpp::estimate_plane(const vector<PointXYZI> &ground) {
    ...
    Eigen::MatrixX4f eigen_ground(ground.size(), 4);
    int j=0;
    for (auto &p: ground) {
        eigen_ground.row(j++) << p.x, p.y, p.z, p.intensity;
    }
    Eigen::MatrixX4f centered = eigen_ground.rowwise() - eigen_ground.colwise().mean();
    Eigen::MatrixX4f cov = (centered.adjoint() * centered) / double(eigen_ground.rows() - 1);

    pc_mean_.resize(3);
    pc_mean_ << eigen_ground.colwise().mean()(0), eigen_ground.colwise().mean()(1), eigen_ground.colwise().mean()(2);

    Eigen::JacobiSVD<Eigen::MatrixX4f> svd(cov, Eigen::DecompositionOptions::ComputeFullU);
    ...
    for (int zone_idx = 0; zone_idx < params_.num_zones; ++zone_idx) {
        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
                ...
                centers_.push_back(PointXYZI(pc_mean_(0), pc_mean_(1), pc_mean_(2), pc_mean_(3)));
                normals_.push_back(PointXYZI(normal_(0), normal_(1), normal_(2), normal_(3)));
...
void PatchWorkpp::pc2czm(const Eigen::MatrixXf &src, std::vector<Zone> &czm) {
    ...
    for (int i=0; i<src.rows(); i++) {
        ...
        float x = src.row(i)(0), y = src.row(i)(1), z = src.row(i)(2), inten = src.row(i)(3);
        ...
        if ((r <= max_range) && (r > min_range)) {
            ...
            if (r < min_range_1) { // In First rings
                ...
                czm[0][ring_idx][sector_idx].emplace_back(PointXYZI(x, y, z, inten));
                ...
                czm[1][ring_idx][sector_idx].emplace_back(PointXYZI(x, y, z, inten));
                ...
                czm[2][ring_idx][sector_idx].emplace_back(PointXYZI(x, y, z, inten));
            } else { // Far!
                ...
                czm[3][ring_idx][sector_idx].emplace_back(PointXYZI(x, y, z, inten));
            }

        } else {
            cloud_nonground_.push_back(PointXYZI(x, y, z, inten));
        }
    }
